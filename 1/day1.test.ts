  ...
  describe('part1', () => {
    describe('example data', () => {
      each`
        ...
        ${'-1 -2 -3'}         | ${-6}
    `.it(
        'should apply changes $changes and end up at $expected',
        ({ changes, expected }) => {
          expect(applyChanges(changes)).toBe(expected);
        }
      );
    });

    describe('actual data', () => {
      it('should sum input data', () => {
        expect(applyChanges(inputData)).toBe(479);
      });
    });
  });
  ...
  describe('part2', () => {
    describe('example data', () => {
      each`
        ...
        ${'+7 +7 -2 -7 -4'} | ${14}
      `.it(
        'should return $expected as the first frequency that repeats',
        ({ changes, expected }) => {
          expect(findRepeatingFrequency(changes)).toBe(expected);
        }
      );
    });

    describe('actual data', () => {
      it('should find repeating frequency', () => {
        expect(findRepeatingFrequency(inputData)).toBe(66105);
      });
    });
  });
});
